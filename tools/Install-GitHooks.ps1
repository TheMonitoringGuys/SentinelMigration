...
.DESCRIPTION
    ...
    Why this exists rather than a CI workflow: GitHub Actions does not execute in
    ...
    is valid and will start working if the repo moves to an org, but today it has
    never produced a check run. This hook is the gate that actually runs.
...
Write-Host ''
Write-Host 'Sentinel Migration Assistant - git hooks' -ForegroundColor Cyan
Write-Host ''
...
# clone and in every worktree - each finds its own copy of tools/hooks. An absolute
# path would silently point every worktree at one clone's scripts.
...
# Report on the prerequisites rather than assuming them. A hook that cannot run is
...
Write-Host ''
Write-Host '  Done. The full suite now runs before each push (about 25 seconds).' -ForegroundColor Green
