...
.DESCRIPTION
    ...
    it has never produced a single check run. Until that changes, this hook is the
    only automated thing standing between a broken commit and the remote.

    The suite takes roughly 25 seconds. That is the entire cost of the gate.
...
# The suite writes progress banners to the host on purpose; swallow them here so
# the hook output stays readable, and surface only the failures that matter.
